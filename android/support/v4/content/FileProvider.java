...
public class FileProvider extends ContentProvider {
    ...
    static class SimplePathStrategy implements PathStrategy {
        ...
        public Uri getUriForFile(File file) {
            try {
                ...
                if (rootPath.endsWith(MqttTopic.TOPIC_LEVEL_SEPARATOR)) {
                    path = path.substring(rootPath.length());
                } else {
                    path = path.substring(rootPath.length() + 1);
                }
                return new Builder().scheme("content").authority(this.mAuthority).encodedPath(Uri.encode((String) mostSpecific.getKey()) + '/' + Uri.encode(path, MqttTopic.TOPIC_LEVEL_SEPARATOR)).build();
            } catch (IOException e) {
    ...
    static {
        ...
        DEVICE_ROOT = new File(MqttTopic.TOPIC_LEVEL_SEPARATOR);
        sCache = new HashMap();
    }
